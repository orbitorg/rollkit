...
import (
	...
	cmtrand "github.com/rollkit/rollkit/libs/rand"
	"github.com/rollkit/rollkit/libs/test"
)
...
func TestMempoolConcurrentUpdateAndReceiveCheckTxResponse(t *testing.T) {
	...
	for h := 1; h <= 100; h++ {
		...
		go func(h int) {
			...
			err := mp.FlushAppConn()
			require.NoError(t, err)
			err = mp.Update(uint64(h), []types.Tx{tx}, abciResponses(1, abci.CodeTypeOK), nil, nil)
			mp.Unlock()
			require.NoError(t, err)
			require.Equal(t, uint64(h), mp.height.Load(), "height mismatch")
		}(h)
...
func TestMempoolConcurrentCheckTxAndUpdate(t *testing.T) {
	...
	go func() {
		...
		for h := 1; h <= maxHeight; h++ {
			...
			require.NoError(t, err)
			err = mp.Update(uint64(h), txs, abciResponses(len(txs), abci.CodeTypeOK), nil, nil)
			require.NoError(t, err)
			mp.Unlock()
		}
	}()
...
func doCommit(t require.TestingT, mp Mempool, app abci.Application, txs types.Txs, height uint64) {
